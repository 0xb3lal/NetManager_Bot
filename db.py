...
def init_db():
    with get_db() as conn:
        ...
        if not _column_exists(conn, "daily_limits", "mode"):
            ...
            logger.info(
                "Migrated 'daily_limits' table: added 'mode' column (persistent/today_only)."
            )
...
def set_device_allowed(mac: str, allowed: bool) -> bool:
    ...
    try:
        with get_db() as conn:
            conn.execute("UPDATE devices SET allowed = ? WHERE mac = ?", (val, mac))
    ...
    try:
        with get_db() as conn:
            ...
            if row is None:
                ...
                logger.info(
                    f"New device discovered and saved as PENDING: {mac} ({hostname})"
                )
                return True
...
def set_onboarding_confirmed(mac: str):
    ...
    try:
        with get_db() as conn:
            conn.execute(
                "UPDATE devices SET onboard_status = 'confirmed' WHERE mac = ?", (mac,)
            )
...
def mark_anomaly_handled(mac: str):
    ...
    try:
        with get_db() as conn:
            conn.execute("UPDATE devices SET anomaly_handled = 1 WHERE mac = ?", (mac,))
    except Exception as e:
...
def update_hostname(mac: str, hostname: str) -> bool:
    ...
    try:
        ...
        if updated:
            logger.info(
                f"Hostname updated for {mac}: '{row['hostname']}' -> '{hostname}' (via update_hostname)"
            )
        return bool(updated)
    except Exception as e:
    ...
    try:
        with get_db() as conn:
            exists_old = conn.execute(
                "SELECT 1 FROM devices WHERE mac=?", (old_mac,)
            ).fetchone()
            if not exists_old:
                return False
            exists_new = conn.execute(
                "SELECT 1 FROM devices WHERE mac=?", (new_mac,)
            ).fetchone()
            if exists_new:
                return False
            for tbl in (
                ...
                try:
                    row_new = conn.execute(
                        f"SELECT 1 FROM {tbl} WHERE mac=?", (new_mac,)
                    ).fetchone()
                    if row_new:
                        return False
                except Exception:
            ...
            for tbl in (
                ...
                try:
                    conn.execute(
                        f"UPDATE {tbl} SET mac=? WHERE mac=?", (new_mac, old_mac)
                    )
                except Exception:
                    pass
...
def get_ban_reason(mac: str) -> str | None:
    mac = mac.upper()
    with get_db() as conn:
        row = conn.execute("SELECT reason FROM banned WHERE mac = ?", (mac,)).fetchone()
...
def get_device_daily_limit(mac: str) -> float | None:
    ...
    try:
        with get_db() as conn:
            row = conn.execute(
                "SELECT limit_gb, mode, expires_on FROM daily_limits WHERE mac = ?",
                (mac,),
            ).fetchone()
        if not row:
            return None
...
def get_device_daily_limit_with_mode(
    mac: str,
...
def set_device_daily_limit(mac: str, value: float, mode: str = "persistent"):
    ...
    try:
        ...
        logger.info(
            f"Daily limit for {mac} set to {value} GB mode={mode} expires_on={expires_on}"
        )
    except Exception as e:
...
def get_all_device_daily_limits() -> dict:
    ...
    try:
        with get_db() as conn:
            rows = conn.execute(
                "SELECT mac, limit_gb, mode, expires_on FROM daily_limits"
            ).fetchall()
        out = {}
        for row in rows:
            mode = row["mode"] if "mode" in row.keys() and row["mode"] else "persistent"
    ...
    try:
        ...
        with get_db() as conn:
            ...
            if not expired:
                return 0
            conn.execute(
                "DELETE FROM daily_limits WHERE mode='today_only' AND expires_on != ?",
                (today,),
            )
            deleted = conn.execute("SELECT changes() as c").fetchone()["c"]
        if deleted:
            logger.info(
                f"Cleared {deleted} expired today_only daily limit(s) (today={today})."
            )
        return deleted
    except Exception as e:
...
def clear_all_device_daily_limits() -> int:
    try:
        with get_db() as conn:
            count = conn.execute("SELECT COUNT(*) as c FROM daily_limits").fetchone()[
                "c"
            ]
            conn.execute("DELETE FROM daily_limits")
        if count:
            logger.info(
                f"Cleared {count} custom daily limit override(s) at midnight reset."
            )
        return count
    except Exception as e:
