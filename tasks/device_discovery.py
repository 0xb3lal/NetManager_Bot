...
def setup_device_discovery_task(bot):
        ...
        for attempt in range(1, 3):
            try:
                ...
                if attempt == 1:
                    ...
                    continue
                logger.exception(f"Error in device discovery task after retry: {e}")
                break
                ...
                logger.exception(f"Error in device discovery task: {e}")
                break
    ...
    async def before_device_discovery():
        ...
        logger.info(
            "Device discovery task started " "(checks for new devices every 5 minutes)."
        )
