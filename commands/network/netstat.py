...
def setup(bot):
    ...
    @bot.tree.command(
        name="netstat",
        description="Show device network usage or daily limit status"
    )
    ...
    async def netstat(
        ...
        try:
            ...
            async with ROUTER_LOCK:
                ...
                speed_history = await asyncio.to_thread(
                    get_speed_history
                )

                daily_history = await asyncio.to_thread(
                    get_daily_history
                )

                dhcp_leases, _, _ = await asyncio.to_thread(
                    fetch_devlist
                )
            ...
            if view_value == "daily_limit":
                ...
                for mac, info in devices_info.items():
                    ...
                    total_bytes = combined_usage.get(
                        info["ip"],
                        0
                    )

                    usage_gb = bytes_to_mb(
                        total_bytes
                    ) / 1024
                ...
                if combined_data:
                    ...
                    embed = discord.Embed(
                        description="✨ No devices found in history.",
                        color=0x95a5a6
                    )
            ...
            logger.error(
                f"Error in netstat: {e}"
            )

            try:
                await interaction.followup.send(
                    "`❌` Error compiling network status."
                )
