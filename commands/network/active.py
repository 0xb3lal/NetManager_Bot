...
def setup(bot):
    ...
    async def active(interaction: discord.Interaction):
        ...
        logger.info(
            f"ACTION: /active | User: {interaction.user}"
        )
        ...
        try:
            ...
            for mac, info in devices_info.items():
                ...
                is_online = (
                    mac in active_signals
                    and mac in arp_active
                )
                ...
                combined_data.append(
                    {
                        ...
                        "online_sort": (
                            1
                            if is_online and not is_banned
                            else 0
                        )
                    }
                )
            ...
            logger.info(
                f"SUCCESS: /active | Sent device status to {interaction.user}"
            )

        except Exception as e:

            logger.error(
                f"FAILURE in /active command: {e}"
            )
