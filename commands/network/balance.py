...
def setup(bot):
    ...
    async def balance(interaction: discord.Interaction):
        ...
        try:
            try:
                traffic = await asyncio.to_thread(fetch_radius_traffic, True)
            except Exception as e:
                logger.error(f"Error in balance-fetch thread: {e}")
                traffic = None
            ...
            if traffic:
                ...
                embed = discord.Embed(
                    title="`📊` Network Status", description=status_box, color=0x3498DB
                )

                await interaction.followup.send(embed=embed)

                logger.info(f"SUCCESS: /balance | Balance sent: {traffic}")

            else:
                embed = discord.Embed(
                    title="`❌` System Error",
                    description=(
                        "`Could not fetch balance. " "Radius server unreachable.`"
                    ),
                    color=0xE74C3C,
                )
                await interaction.followup.send(embed=embed)
                logger.error("FAILURE: /balance | Could not fetch traffic")
