...
from router.static_leases import is_valid_hostname, is_valid_ip, set_static_hostname
...
def setup(bot):
    ...
    @bot.tree.command(
        name="rename", description="Rename a device's hostname via router static lease"
    )
    ...
    async def rename(
        interaction: discord.Interaction, mac: str, name: str, ip: str = None
    ):
        ...
        try:
            ...
            if explicit_ip:
                ...
                if not resolved_ip:
                    try:
                        ...
                        logger.warning(
                            f"Static DHCP lookup failed for {mac_upper}: {e}"
                        )
            if not resolved_ip:
                await interaction.followup.send(
                    embed=error_embed(
            ...
            if not success:
                logger.error(
                    f"/rename failed for {mac_upper} -> {name} ({resolved_ip}): {err}"
                )
            ...
            try:
                ...
                logger.warning(
                    f"Failed to sync DB/state hostname after rename for {mac_upper}: {e}"
                )
            try:
                async with ROUTER_LOCK:
            ...
            await interaction.followup.send(embed=embed)
            logger.info(
                f"/rename success {mac_upper} -> {name} ({resolved_ip}) by {interaction.user}"
            )
