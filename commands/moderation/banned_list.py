...
def setup(bot):
    ...
    @bot.tree.command(name="banned_list", description="List currently banned MACs")
    async def list_banned(interaction: discord.Interaction):

        logger.info(f"ACTION: /banned_list | User: {interaction.user}")
        ...
        try:
            ...
            if state.banned_macs:
                ...
                for m in state.banned_macs:
                    ...
                    device_name = state.macs_list.get(m, "Unknown Device")
                ...
                if manual_lines:
                    ...
                    sections += [
                        f"  {i:02d}. {name}" for i, name in enumerate(manual_lines, 1)
                    ]
                ...
                if daily_lines:
                    ...
                    sections += [
                        f"  {i:02d}. {name}" for i, name in enumerate(daily_lines, 1)
                    ]
