...
def setup(bot):
    ...
    @bot.tree.command(
        name="macs",
        description="List or manage known devices (list/edit/remove)"
    )
    ...
    async def macs(
        ...
        try:
            ...
            logger.info(f"SUCCESS: /macs list | Returned {len(state.macs_list)} devices")
        except Exception as e:
...
async def _handle_edit(interaction: discord.Interaction, mac: str, new_mac: str):
    ...
    if not mac or not new_mac:
        await interaction.followup.send("`❌` Both `mac` (old) and `new_mac` are required for edit.")
        return
    old = mac.strip().upper()
    ...
    if not is_valid_mac(new):
        ...
        return
    if old == new:
        await interaction.followup.send("`ℹ️` Old and new MAC are identical — no change.")
        return
    ...
    try:
        ...
        if new in _sessions:
            await interaction.followup.send(f"`❌` New MAC already has an active onboarding session: `{new}`")
            return
    except Exception:
        pass
    ...
    try:
        ...
        if new_static_exists:
            await interaction.followup.send(f"`❌` New MAC already has a static DHCP entry: `{new}`")
            return
    except Exception as e:
...
async def _handle_remove(interaction: discord.Interaction, mac: str):
    ...
    if not __import__("utils.validators", fromlist=["is_valid_mac"]).is_valid_mac(target):
    ...
    try:
        ...
        async with ROUTER_LOCK:
            ...
            if has_static:
                ...
                ok, err = await asyncio.to_thread(_push_dhcpd_static, new_raw)
                if not ok:
                    logger.warning(f"Static DHCP cleanup failed for remove {target}: {err}")
    except Exception as e:
    ...
    logger.info(f"SUCCESS: /macs remove {target} by {interaction.user} was_in_macs={was_in_macs} db_success={db_success}")
