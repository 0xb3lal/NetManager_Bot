...
def __getattr__(name: str):
    ...
    if name in ("OnboardingQ1View", "OnboardingQ2View", "OnboardingQ3View", "_OnboardingBaseView"):
    ...
    if name in ("_handle_router_failure", "_handle_rename_failure", "_run_router_with_retry"):
        mod = importlib.import_module("services.onboarding_retry")
        return getattr(mod, name)
    # Embeds / helpers
    if name in ("_info_box", "_kickoff_embed", "_whitelist_question_embed", "_name_question_embed", "_allowed_ack_embed", "_blocked_ack_embed", "_permanent_failure_embed", "_processing_embed", "_COLOR_NEW", "_COLOR_OK", "_COLOR_BLOCK"):
    ...
    if name in ("ONBOARDING_STEP_TIMEOUT", "OnboardingSession", "_sessions", "drop_session", "_device_gone", "_gone_notice", "_device_name"):
