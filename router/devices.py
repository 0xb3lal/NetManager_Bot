...
def fetch_devlist_and_discover(bot_instance):
    ...
    from config import RSSI_DISPLAY_ENABLED, DISTANCE_ESTIMATION_ENABLED, RSSI_AT_1M_DBM, WIFI_PATH_LOSS_EXPONENT
    from utils.wireless import rssi_to_quality_pct, rssi_to_distance_m, clamp_distance

    def _build_rssi_map(devs) -> dict[str, int]:
        try:
            return {dev[1].upper(): int(dev[2]) for dev in devs if len(dev) > 2 and isinstance(dev[2], (int, float))}
    ...
    for lease in dhcp_leases:
        ...
        def _wireless_info(rssi_val: int | None) -> tuple[int | None, float | None]:
            ...
            if DISTANCE_ESTIMATION_ENABLED:
                raw = rssi_to_distance_m(rssi_val, RSSI_AT_1M_DBM, WIFI_PATH_LOSS_EXPONENT)
                if raw is not None:
                    clamped, was_clamped = clamp_distance(raw, 1.0)
                    if was_clamped:
                        logger.debug(f"Distance for {mac} clamped {raw:.2f}m -> 1.0m (RSSI {rssi_val} dBm)")
                    d = clamped
            return q, d
    ...
    if new_devices and bot_instance.loop.is_running():
        async def _onboard_new_devices():
            ...
            for mac, hostname, ip, rssi, distance, quality in new_devices:
                await start_onboarding(bot_instance, mac, hostname, ip, rssi, distance, quality)
