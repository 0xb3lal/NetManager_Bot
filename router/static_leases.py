...
from config import ROUTER_URL, ROUTER_SESSION
...
def _parse_raw_entries(raw: str):
    ...
    for ent_raw in entries:
        ...
        j = 0
        while j < len(ent_raw):
            if ent_raw[j] == "\\" and j+1 < len(ent_raw) and ent_raw[j+1] in ("<", ">", ":", "\\"):
                # keep escaped
        ...
        parsed.append({"raw": ent_raw, "mac": mac, "ip": ip, "hostname": hostname, "flag": flag})
    return parsed
...
def fetch_current_entries():
    ...
    if raw is None:
        raise RuntimeError("Failed to fetch current dhcpd_static from router (nvram get failed)")
    # raw may be empty
...
def _push_dhcpd_static(new_raw: str, timeout=30):
    ...
    try:
        resp = ROUTER_SESSION.post(f"{ROUTER_URL}/tomato.cgi", headers=headers, data=data, timeout=timeout)
...
def set_static_hostname_sync(mac: str, ip: str, hostname: str):
    ...
    if not is_valid_hostname(hostname):
        return False, f"Invalid hostname '{hostname}' (1-32 chars, alphanumeric/hyphen, must start/end alnum)"
    mac = mac.upper()
    try:
        ...
        try:
            ...
            try:
                ...
                if not got:
                    ...
                    logger.warning(f"set_static_hostname attempt {attempt}/{max_attempts} lock busy for {mac}")
                    # treat as retryable failure
                else:
                    acquired = True
                    success, err = await asyncio.to_thread(set_static_hostname_sync, mac, ip, hostname)
                    if success:
                        return True, None
                    last_err = err
                    logger.warning(f"set_static_hostname attempt {attempt}/{max_attempts} failed for {mac}: {err}")
            except ImportError:
                # fallback
